...
export function SingleProjectView({ project, initialMediaKey, interactive = true }: SingleProjectViewProps) {
  ...
  const safeCurrentImageIndex = Math.min(
    currentImageIndex,
    Math.max(filteredMedia.length - 1, 0),
  );
  ...
  const activeMediaFilterLabel = mediaFilterOptions.find((option) => option.value === mediaFilter)?.label ?? mediaFilterOptions[0].label;
  const activeMediaFilterCount = mediaFilter === 'all'
    ? allMedia.length
  ...
  return (
    <>
                ...
                <div className="filter-bar-summary">
                  ...
                  {!isMediaFilterOpen && (
                    <span className="filter-bar-current" aria-live="polite">
                      {activeMediaFilterLabel} <span className="filter-bar-count">[{activeMediaFilterCount}]</span>
                    </span>
                  )}
                </div>
                {isMediaFilterOpen && (
                ...
                {filteredMedia.map((media, index) => {
                  ...
                  return (
                    <motion.button
                      ...
                      transition={{duration: reducedMotion ? 0 : 0.44, delay: reducedMotion ? 0 : Math.min(index, 8) * 0.035, ease: [0.22, 1, 0.36, 1]}}
                    >
      ...
      {lightboxOpen && interactive && createPortal((
        <div className="lightbox" role="dialog" aria-modal="true" aria-label={`${project.title} media viewer`} onClick={handleLightboxClose}>
          ...
          <button
            ...
            onClick={(e) => { e.stopPropagation(); setCurrentImageIndex((prev) => (prev > 0 ? prev - 1 : filteredMedia.length - 1)); }}
          >
            &lt;
          </button>
          <button
            ...
            onClick={(e) => { e.stopPropagation(); setCurrentImageIndex((prev) => (prev < filteredMedia.length - 1 ? prev + 1 : 0)); }}
          >
            &gt;
          </button>
